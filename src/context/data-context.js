...
export const DataContextProvider = (props) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await axios.get(
        `https://task-manager-app-uvhv.onrender.com/task/all`
      );
  ...
  const deleteTask = async (id) => {
    ...
    console.log(id);
    await axios.delete(
      `https://task-manager-app-uvhv.onrender.com/task/delete/${id._id}`
    );
  };
  ...
  const updateTask = async (id) => {
    ...
    await axios.put(
      `https://task-manager-app-uvhv.onrender.com/task/update/${id._id}`,
      {
        status: "complete",
      }
    );
